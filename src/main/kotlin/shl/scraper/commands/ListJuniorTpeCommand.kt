...
class ListJuniorTpeCommand : ListThreadsCommand() {
    override fun buildResponseEmbed(
        event: MessageReceivedEvent, arguments: List<String>,
        userRole: UserRole, discord: Discord
    ): EmbedBuilder? {
        val pt = super.buildResponseEmbed(event, listOf("66"), UserRole.REGULAR, discord)
    ...
    override fun shouldShow(thread: ForumThread): Boolean {
        return thread.open && !thread.pinned && !thread.name.matches("(?i)\\s*S\\d+ SMJHL milestones\\s*".toRegex())
    }

    override fun commandNameRegex(): Regex {
        return "(?i)!tpej".toRegex()
    }
